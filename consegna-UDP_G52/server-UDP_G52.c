...
int main() {
    ...
    while (1) {
        ...
        sendto(sock, "connessione avvenuta",
               strlen("connessione avvenuta") + 1, 0,
               (struct sockaddr *)&client, lunghezza);

        switch(lettera) {
            case 'a': case 'A':
                strcpy(buffer, "ADDIZIONE");
                break;
            case 's': case 'S':
                strcpy(buffer, "SOTTRAZIONE");
                break;
            case 'm': case 'M':
                strcpy(buffer, "MOLTIPLICAZIONE");
                break;
            case 'd': case 'D':
                strcpy(buffer, "DIVISIONE");
                break;
            default:
